...
if [ -z "$DEVICE_UDID" ]; then
    ...
    [ -z "$DEVICE_UDID" ] && error "No iPhone found. Connect your iPhone and trust this Mac."
    info "Found device: ${DEVICE_NAME:-$DEVICE_UDID}"
fi
...
INSTALL_TARGET="${DEVICE_NAME:-$DEVICE_UDID}"
if xcrun devicectl device install app --device "$INSTALL_TARGET" "$APP_PATH" 2>/dev/null; then
